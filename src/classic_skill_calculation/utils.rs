...
pub fn get_last_tick_time(hit_obj: &structs::HitObject) -> i64 {
    if hit_obj.ticks.len() == 0 {
        if hit_obj.repeat > 1 {
            ...
            return (hit_obj.end_time as f64 - (hit_obj.end_time - last_in_vec) as f64 / 2.0) as i64;
        } else {
            return (hit_obj.end_time as f64 - (hit_obj.end_time - hit_obj.time) as f64 / 2.0) as i64;
        }
    } else {
...
fn apply_speed(mut beatmap: structs::Beatmap, speed: f64) -> structs::Beatmap {
    ...
    while i < beatmap.timing_points.len() - 1 {
        ...
        beatmap.timing_points[i].offset = (f64::ceil(beatmap.timing_points[i].offset as f64 / speed)) as i32 as f64;
