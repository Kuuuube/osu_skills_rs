...
fn calculate_movement_data(mut beatmap: structs::Beatmap) -> structs::Beatmap {
    ...
    while i < beatmap.hit_objects.len() {
        if (calculation_utils::is_hit_object_type(&beatmap.hit_objects[i].hit_object_type, structs::HitObjectType::Normal) || calculation_utils::is_hit_object_type(&beatmap.hit_objects[i].hit_object_type, structs::HitObjectType::Slider)) && previous_time != -1 {
            ...
            let rad_subtract: f64 = 2.0 * calculation_utils::cs_to_px(beatmap.cs) as f64;
    ...
    for hit_obj in &beatmap.hit_objects {
        ...
        if calculation_utils::is_hit_object_type(&hit_obj.hit_object_type, structs::HitObjectType::Normal) {
            ...
            for tick in &hit_obj.ticks {
                target_point = structs::Timing{time: *tick as i64, pos: skill_calculation::calculation::slider::get_slider_pos(hit_obj, *tick), key: i, press: true, data: Default::default()};
...
fn gather_aim_points(mut beatmap: structs::Beatmap) -> structs::Beatmap {
    for hit_obj in &beatmap.hit_objects {
        if calculation_utils::is_hit_object_type(&hit_obj.hit_object_type, structs::HitObjectType::Normal) {
            ...
            if hit_obj.ticks.len() != 0 || pair_structs::get_distance_from(&hit_obj.pos, &end_pos) > 2.0 * calculation_utils::cs_to_px(beatmap.cs) as f64 {
        ...
        if calculation_utils::is_hit_object_type(&beatmap.hit_objects[i].hit_object_type, structs::HitObjectType::Slider) {
            ...
            match beatmap.hit_objects[i].curve_type {
                structs::CurveType::BezierCurve => {
                    let slider_data: structs::Slider = skill_calculation::calculation::slider::slider_fn(beatmap.hit_objects[i].clone(), false);
                ...
                structs::CurveType::PerfectCurve => {
                    if beatmap.hit_objects[i].curves.len() == 2 {
                        let circle_data: structs::CircumscribedCircle = skill_calculation::calculation::slider::circumscribed_circle(beatmap.hit_objects[i].clone());
                ...
                structs::CurveType::LinearCurve => {
                    let slider_data: structs::Slider = skill_calculation::calculation::slider::slider_fn(beatmap.hit_objects[i].clone(), true);
                    beatmap.hit_objects[i].lerp_points.resize(slider_data.curve.len(), Default::default());
                    beatmap.hit_objects[i].lerp_points = slider_data.curve;
                    beatmap.hit_objects[i].ncurve = slider_data.ncurve;
                },
...
fn gather_tap_patterns(mut beatmap: structs::Beatmap) -> structs::Beatmap {
    ...
    while i < new_press_intervals.len() {
        let is_found: bool = uniq.contains(&new_press_intervals[i]);
        if is_found == false {
            let mut found: bool = false;
            let mut p: i32 = new_press_intervals[i] - offset_max_displacement;
            while p <= new_press_intervals[i] + offset_max_displacement {
                let is_found_p = uniq.contains(&p);
                if is_found_p == true {
                    new_press_intervals[i] = p;
                    found = true;
                    break;
                }
                p += 1;
