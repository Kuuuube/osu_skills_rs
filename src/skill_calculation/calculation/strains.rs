...
pub fn calculate_tap_strains(mut beatmap: structs::Beatmap) -> structs::Beatmap {
    ...
    for interval in &beatmap.press_intervals {
        ...
        if c == 0 {
            let largest_interval: f64 = SKILL_CALCULATION_VARS.stamina.largest_interval;
            if interval_deref >= largest_interval as i32 {
                strain = 0.0;
            } else {
                let scale: f64 = SKILL_CALCULATION_VARS.stamina.scale;
                let pow: f64 = SKILL_CALCULATION_VARS.stamina.pow;
                let mult: f64 = SKILL_CALCULATION_VARS.stamina.mult;
                strain = scale
                    / f64::powf(
            ...
            let largest_interval: f64 = SKILL_CALCULATION_VARS.stamina.largest_interval;
            if interval_deref >= largest_interval as i32 {
                let decay_max: f64 = SKILL_CALCULATION_VARS.stamina.decay_max;
                strain *= decay_max;
            } else {
                ...
                let decay: f64 = SKILL_CALCULATION_VARS.stamina.decay;
...
fn get_weighted_aim_distance(distance: f64) -> f64 {
    let dist_mult: f64 = SKILL_CALCULATION_VARS.agility.dist_mult;
    let dist_pow: f64 = SKILL_CALCULATION_VARS.agility.dist_pow;
    let dist_div: f64 = SKILL_CALCULATION_VARS.agility.dist_divisor;
...
fn get_weighted_aim_time(time: f64) -> f64 {
    let time_mult: f64 = SKILL_CALCULATION_VARS.agility.time_mult;
    let time_pow: f64 = SKILL_CALCULATION_VARS.agility.time_pow;
...
pub fn calculate_aim_strains(mut beatmap: structs::Beatmap) -> structs::Beatmap {
    ...
    while i < beatmap.aim_points.len() {
        ...
        if i != 0 {
            ...
            if i > 1 {
                let angle_mult: f64 = SKILL_CALCULATION_VARS.agility.angle_mult;
            ...
            {
                let slider_strain_decay: f64 = SKILL_CALCULATION_VARS.agility.slider_strain_decay;
                strain *= slider_strain_decay;
            }

            let strain_decay: f64 = SKILL_CALCULATION_VARS.agility.strain_decay;
