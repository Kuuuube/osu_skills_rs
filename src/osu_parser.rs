use crate::{algs::md5, pair_structs, structs};
use std::io::BufRead;
...
pub fn parse_beatmap(file_path: std::path::PathBuf) -> structs::Beatmap {
    ...
    if file_data.len() >= 3 {
        ...
        if magic_number != vec![0xEF, 0xBB, 0xBF] /* UTF-8 */ && magic_number != vec![0x6F, 0x73, 0x75]
        /* osu */
        {
            return beatmap_data;
        }
    ...
    for (_index, line) in reader.lines().enumerate() {
        let line_unwrap = match line {
            ...
            Err(error) => {
                print!(
                    "osu!Skills rs: failed to parse .osu file. Error: {}. Path: `{}`\n\n",
                    error,
        ...
        match &line_unwrap as &str {
            "[General]" => {
                found = Found::FoundGeneral;
                is_header = true;
            }
            "[Editor]" => {
                found = Found::FoundEditor;
                is_header = true;
            }
            "[Metadata]" => {
                found = Found::FoundMetadata;
                is_header = true;
            }
            "[Difficulty]" => {
                found = Found::FoundDifficulty;
                is_header = true;
            }
            "[Events]" => {
                found = Found::FoundEvents;
                is_header = true;
            }
            "[TimingPoints]" => {
                found = Found::FoundTimingPoints;
                is_header = true;
            }
            "[Colours]" => {
                found = Found::FoundColours;
                is_header = true;
            }
            "[HitObjects]" => {
                found = Found::FoundHitobjects;
                is_header = true;
            }
            _ => {
                is_header = false;
        ...
        if !is_header {
            match found {
                ...
                Found::FoundDifficulty => {
                    beatmap_data = difficulty_parser(beatmap_data, line_unwrap)
                }
                Found::FoundEvents => {}
                Found::FoundTimingPoints => {
                    beatmap_data = timing_points_parser(beatmap_data, line_unwrap)
                }
                Found::FoundColours => {}
                Found::FoundHitobjects => {
                    beatmap_data = hit_objects_parser(beatmap_data, line_unwrap)
                }
                Found::FoundNone => {}
            };
        }
...
fn difficulty_parser(mut beatmap: structs::Beatmap, line: String) -> structs::Beatmap {
    ...
    fn osu_file_format_3_7(split: Vec<&str>, mut beatmap: structs::Beatmap) -> structs::Beatmap {
        match split[0] {
            ...
            "OverallDifficulty" => {
                beatmap.od = safe_parse_f64(split[1]);
                beatmap.ar = safe_parse_f64(split[1]);
            }
        ...
        match match_hit_object_type {
            ...
            structs::HitObjectType::Slider => {
                ...
                while i < slider_split.len() {
                    ...
                    let curve = pair_structs::Pairf64 {
                        x: safe_parse_f64(curve_split[0]),
                        y: safe_parse_f64(curve_split[1]),
                    };
...
fn safe_parse_f64(input: &str) -> f64 {
    let output = match input.parse::<f64>() {
        ...
        Err(error) => {
            print!(
                "osu!Skills rs: failed to parse f64 in .osu file. Error: {}: `{}`\n\n",
                error, input
            );
            -1.0
        }
...
fn safe_parse_i32(input: &str) -> i32 {
    let output = match input.parse::<i32>() {
        ...
        Err(error) => {
            print!(
                "osu!Skills rs: failed to parse i32 in .osu file. Error: {}: `{}`\n\n",
                error, input
            );
            -1
        }
...
fn safe_parse_i64(input: &str) -> i64 {
    let output = match input.parse::<i64>() {
        ...
        Err(error) => {
            print!(
                "osu!Skills rs: failed to parse i64 in .osu file. Error: {}: `{}`\n\n",
                error, input
            );
            -1
        }
